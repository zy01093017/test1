...
public class QuicksortActor extends TestableActor {
    ...
    protected void loopBody(Message m) {
        try {
            ...
            if ("sort".equals(subject)) {
                ...
                logger.warn("QuicksortActor:{} loopBody unknown subject: {}", getName(), subject);
            }
        } catch (Exception e) {
    ...
    private void doSort(Message m, ActorManager manager) {
        ...
        if (values != null) {
            logger.info("receive sort values ({}): {}, {}", id, nest, values);
            doSortWorker(manager, nest, values, id, pm);
        } else if (pm != null) {
            logger.info("receive sort pm {}({}): count={}/{}/{}, {}<{}>{}", id, nest, pm.count, pm.lessNeeded,
                        pm.moreNeeded, pm.less, pm.pivot, pm.more);
            if (pm.count > 0) {
                if (pm.lessNeeded) {
                    logger.info("receive sort less ({}): {}, {}", id, nest, pm.less);
                    pm.lessNeeded = false;
                    pm.count--;
                    doSortWorker(manager, nest + 1, pm.less, pm.id, pm);
                } else if (pm.moreNeeded) {
                    logger.info("receive sort more ({}): {}, {}", id, nest, pm.more);
                    pm.moreNeeded = false;
                    pm.count--;
                ...
                if (pm.count == 0) {
                    ...
                    logger.info("send merge {}-{}({}): {}<{}>{}", id, pm.id, nest, pm.less, pm.pivot, pm.more);
                    recordSend(id, manager.send(dm, this, getCategoryName()));
                }
            }
        } else {
            logger.error("receive empty sort {}({})", id, nest);
        }
        recordComplete(id, 1);
    }
    ...
    protected void doSortWorker(ActorManager manager, int nest, List<Comparable> values, String id, PendingMerge pm) {
        ...
        if (values.size() <= 1) {
            ...
            logger.info("send size complete {}({}): {}", id, nest, values);
            recordSend(id, manager.send(dm, this, getCategoryName()));
        } else {
            pm = new PendingMerge();
            ...
            logger.info("send pm sort {}({}): count={}, {}<{}>{}", id, nest, pm.count, pm.less, pm.pivot, pm.more);
            if (pm.count > 0) {
                if (pm.lessNeeded) {
                    DefaultMessage lm = new DefaultMessage("sort", new Object[] {nest + 1, null, pm.id, pm});
                    logger.info("send sort less {}: {}", pm.id, pm.less);
                ...
                if (pm.moreNeeded) {
                    ...
                    logger.info("send sort more {}: {}", pm.id, pm.more);
    ...
    protected void doMerge(Message m) {
        ...
        String id = params.length > 2 ? (String) params[2] : null;
        pm.merge();
        logger.info("merge {}({}): {} <= {}<{}>{}", pm.id, nest, pm.merge, pm.less, pm.pivot, pm.more);
        DefaultMessage dm = new DefaultMessage("sortComplete", new Object[] {nest, pm.merge, pm.id});
        logger.info("send sort complete {}({})", pm.id, nest);
    ...
    protected void doSortComplete(Message m) {
        ...
        logger.info("receive complete {}({}): {}", id, nest, values);
        recordComplete(id, 1);
        if (isComplete(id)) {
            if (nest == 0) {
                logger.trace("**** Sort {} complete: {}", id, values);
            }
        }
    }
    ...
    protected int recordComplete(String id, int count) {
        ...
        if (res < 0) {
            logger.error("**** recordComplete {} count < 0: {}", id, res);
        } else {
