...
public class ProducerActor extends TestableActor {
    ...
    public void activate() {
        logger.trace("ProducerActor activate: {}", this);
        super.activate();
    ...
    public void deactivate() {
        logger.trace("ProducerActor deactivate: {}", this);
        if (messages.size() > 0) {
    ...
    protected void loopBody(Message m) {
        ...
        if ("produceN".equals(subject)) {
            ...
            if (count > 0) {
                ...
                logger.trace("ProducerActor:{} produceN {} x {}", getName(), count, type);
            ...
            if (count > 0) {
                ...
                // getName(), count, type, messages.size());
                logger.trace("ProducerActor:{} produce1 {} x {}", getName(), count, type);
            ...
            logger.trace("ProducerActor:{} constructionComplete {} from {}", getName(), type, m.getSource().getName());
            Integer mcount = expected.get(type);
            if (mcount != null) {
                mcount--;
                expected.put(type, mcount);
            ...
            logger.trace("ProducerActor:{} init", getName());
            ...
            logger.warn("ProducerActor:{} loopBody unknown subject: {}", getName(), subject);
        }
    }
